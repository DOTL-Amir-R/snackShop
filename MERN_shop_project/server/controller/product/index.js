...
const handelCreateProduct = async (req, res) => {
  ...
  if (!req.body.weghit) return res.status(400).json({ massage: "heyyyy noob weghit kooo!?" })
  if (!req.body.desc) return res.status(400).json({ massage: "heyyyy noob desc kooo!?" })
  if (!req.body.picture) return res.status(400).json({ massage: "heyyyy noob picture kooo!?" })
